...
public class TwentyFour extends SolutionTemplate<Integer, Integer> {
    ...
    Solution<Integer> partTwo() {
        return new BlizzardMap() {
            ...
            public Integer result() {
                ...
                int there = timeTaken(maxY, maxX, toTry);

                final Coordinate saved = this.end;
                final Coordinate end = this.start;

                toTry.clear();
                toTry.put(this.end, new Head(this.end, new Empty()));
                this.end = this.start;

                int back = timeTaken(maxY, maxX, toTry);

                toTry.clear();
                toTry.put(this.start, new Head(this.start, new Empty()));
                this.end = saved;

                int thereAgain = timeTaken(maxY, maxX, toTry);
    ...
    private static class BlizzardMap implements Solution<Integer> {
        protected final Coordinate start = new Coordinate(0, -1);
        ...
        public Integer result() {
            ...
            return timeTaken(maxY, maxX, toTry);
        }

        protected int timeTaken(int maxY, int maxX, Map<Coordinate, Path> toTry) {
            int length = 0;
